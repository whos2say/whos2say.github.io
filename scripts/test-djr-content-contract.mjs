...
const studioFoundationFiles = [studioAuthPlan, studioAuthSchema, studioIndex, studioCallback, studioAuth, studioAuthCallback, studioParticipantDashboard, studioStyles]
...
assert(studioParticipantDashboard.includes(".from('participant_user_access')"), 'Studio dashboard must use participant-scoped access rows as its primary authorization source')
assert(studioParticipantDashboard.includes(".eq('user_id', user.id)"), 'Studio dashboard must explicitly scope participant access rows to the authenticated user')
assert(studioParticipantDashboard.includes(".from('participant_album_access')"), 'Studio dashboard must count participant-scoped album assignments')
...
assert(studioAuth.includes('Registry preview — real access enforcement requires Supabase RLS.'), 'Studio dashboard must label its registry fallback as unenforced preview data')
assert(studioParticipantDashboard.includes('ownerUserIds?.includes(userId)') && studioParticipantDashboard.includes('staffAdminUserIds?.includes(userId)') && studioParticipantDashboard.includes('contributorUserIds?.includes(userId)'), 'Registry preview must filter assignments by the authenticated Supabase user UUID')
assert(!/user\.email|email.*(?:access|role)/i.test(studioParticipantDashboard), 'Studio dashboard must not authorize from Google email identity')
assert(!/contactProfile|socialProfiles|contenteditable|<form/i.test(studioIndex + studioAuth + studioParticipantDashboard), 'Studio dashboard must not expose contact/social or general editing controls')
assert(!studioParticipantDashboard.includes("'/cody/'") && !studioIndex.includes('/cody/'), 'Studio dashboard must not create or link a Cody public route')
