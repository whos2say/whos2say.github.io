/**
 * Vercel Serverless Function — OG-tag shim for Facebook album shares
 * Route: /share/album?album={albumId}   (rewritten from vercel.json)
 *
 ...
 *
 * Regular browser:
 *   → 302 redirect to /album.html?album={albumId} with UTM params
...
function buildOgHtml({ albumId, albumName, coverUrl }) {
  const title        = `${albumName} — Photo Album`
  const description  = "Browse this photo album from Who's 2 Say Foundation!"
  const canonicalUrl = `${SITE_ORIGIN}/share/album?album=${encodeURIComponent(albumId)}`
...
export default async function handler(req, res) {
  ...
  if (!supabaseUrl || !serviceKey) {
    console.error('[share/album] Missing SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY')
    return res.redirect(302, `/album.html?album=${encodeURIComponent(albumId)}`)
  }
  ...
  try {
    ...
    if (albumRes.ok) {
      ...
      console.error('[share/album] album fetch failed:', albumRes.status)
    }
  } catch (err) {
    console.error('[share/album] album fetch error:', err)
  }
  ...
  try {
    ...
    if (photoRes.ok) {
      ...
      console.error('[share/album] photo fetch failed:', photoRes.status)
    }
  } catch (err) {
    console.error('[share/album] photo fetch error:', err)
  }
  ...
  const destination =
    `/album.html?album=${encodeURIComponent(albumId)}` +
